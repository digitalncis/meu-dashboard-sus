...
import unicodedata
import os # Importar para manipulação de arquivos (embora não estritamente necessário se os caminhos forem fixos)
...
st.markdown("""
<style>
    /* ... (CSS Style block mantido para concisão) ... */
...
def identificar_mes_por_arquivo(nome_arquivo):
    ...
    # Se estiver lendo do disco, 'nome_arquivo' será o caminho.
    # Usaremos o nome do arquivo base para identificar o mês, se possível.
    # Exemplo simples: 'data/papa_janeiro_1.csv' -> '1'
    base_name = os.path.basename(nome_limpo) 
    ultimo_digito = base_name[-5] if len(base_name) >= 5 else base_name[-1]
...
@st.cache_data
# A função agora recebe caminhos (strings) em vez de objetos UploadedFile
def load_data_raw(files_papa_paths, files_espelho_paths):
    df_papa = pd.DataFrame()
    if files_papa_paths:
        papa_dfs = []
        for path in files_papa_paths:
            try: 
                df_temp = pd.read_csv(path, sep=',', encoding='latin1', dtype=str)
            except: 
                try: 
                    df_temp = pd.read_csv(path, sep=';', encoding='latin1', dtype=str)
                except Exception as e:
                    st.error(f"Erro ao ler arquivo {path}: {e}")
                    continue

            df_temp['MES_NOME'] = identificar_mes_por_arquivo(path)
    ...
    if files_espelho_paths:
        espelho_dfs = []
        target_files = files_espelho_paths
        for path in target_files:
            try: df_temp = pd.read_csv(path, sep=',', encoding='latin1', dtype=str)
            except: 
                try: df_temp = pd.read_csv(path, sep=';', encoding='latin1', dtype=str)
                except Exception as e:
                    st.error(f"Erro ao ler arquivo {path}: {e}")
...
# --- DEFINIÇÃO DOS CAMINHOS DOS ARQUIVOS (PARA CARREGAMENTO AUTOMÁTICO) ---
# ATENÇÃO: Substitua os nomes dos arquivos pelos nomes reais no seu repositório GitHub
DEFAULT_FILES_PAPA = [
    'data/papa_janeiro.csv',
    'data/papa_fevereiro.csv'
]
DEFAULT_FILES_ESPELHO = [
    'data/espelho_teto_total.csv'
]

# --- SIDEBAR (Reduzida apenas para Filtros se houver dados padrão) ---
with st.sidebar:
    st.image("https://upload.wikimedia.org/wikipedia/commons/thumb/6/68/SUS_Logo.svg/1200px-SUS_Logo.svg.png", width=140)
    st.markdown("### 📥 Dados Carregados")
    st.info("Painel carrega dados automaticamente do repositório.")
    st.markdown("---")
...
# ⚠️ ALTERAÇÃO PRINCIPAL: Carrega os dados padrão
try:
    df_papa_raw, df_teto, dict_procedimentos = load_data_raw(DEFAULT_FILES_PAPA, DEFAULT_FILES_ESPELHO)
    data_loaded_successfully = True
    num_meses_papa = len(DEFAULT_FILES_PAPA)
except Exception as e:
    st.error(f"Erro Crítico ao carregar dados fixos. Verifique os caminhos e o formato dos arquivos CSV no repositório. Erro: {e}")
    data_loaded_successfully = False
    num_meses_papa = 0


if data_loaded_successfully and not df_teto.empty:
    ...
    # Filtro de Unidade na página principal (c_f2)
    ...
    # Mensagem de falha no carregamento dos dados fixos
    st.markdown("""
    <div style='text-align: center; margin-top: 100px; padding: 30px; border: 2px dashed #e74c3c; border-radius: 15px; background-color: #ffe0e0; box-shadow: 0 4px 10px rgba(0,0,0,0.05);'>
        <h2><span style="color: #e74c3c;">Falha no Carregamento Automático!</span></h2>
        <p style='font-size: 1.1rem; color: #c0392b;'>
            O dashboard não conseguiu encontrar ou processar os arquivos de dados fixos.
        </p>
        <p><strong>Verifique se:</strong></p>
        <ul>
            <li>Os arquivos CSV estão na pasta <code>data/</code> do seu repositório GitHub.</li>
            <li>Os nomes dos arquivos em <code>DEFAULT_FILES_PAPA</code> e <code>DEFAULT_FILES_ESPELHO</code> (linhas 421-424) estão corretos.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)